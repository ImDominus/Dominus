...
def trim(tree, minimum, maximum, root = None, count = 0):
    if root is None and count != 0:
        tree.root = None
        return
    elif root is None and count == 0:
        root = tree.root
    count = 1
    if root.key < minimum:
        if root.parent == None:
        ...
        if root.rightchild is not None:
            trim(tree, minimum, maximum, root.rightchild, count)
    elif root.key > maximum:
        if root.parent == None:
        ...
        if root.leftchild is not None:
            trim(tree, minimum, maximum, root.leftchild, count)
    else:
        if root.leftchild is not None:
            trim(tree, minimum, maximum, root.leftchild, count)
        if root.rightchild is not None:
